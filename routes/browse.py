...
def food_list():
    ...
    WHERE f.is_active = 1
      ...
      AND (
          ...
          -- Cross-midnight window example 23:00 → 02:00
          (f.pickup_start > f.pickup_end
           AND (CAST(%s AS TIME) >= f.pickup_start 
                OR CAST(%s AS TIME) <= f.pickup_end))
      )
    ...
    cur.execute(query, (
        ...
        current_time_ist, current_time_ist, current_time_ist
    ))
    ...
    foods = []
    for f in rows:
        restaurant_mrp = float(f["mrp"])
        restaurant_discount = float(f["price"])

        # Platform price = (restaurant discounted price + 15%)
